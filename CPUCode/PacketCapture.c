...
int main( int arc, char** argv )
{
	...
	int burstSizeInBytes = max_get_burst_size(maxfile, "cmd_toMem");

	printf("burstSize: %d\n", burstSizeInBytes);
	printf("burstCount: %d\n", (data_len * sizeof(*data_in))/burstSizeInBytes);

	printf("Running capture.\n");
	PacketCapture_capture(burstSizeInBytes, data_len, data_in);
