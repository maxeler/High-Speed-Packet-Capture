...
int main( int argc, char** argv )
{
	...
	max_file_t *maxfile = PacketCaptureClient_init();
								 ...
								 int ipsB_len)
{
	...
	PacketCaptureClient_enableRemoteCapture_actions_t action =
	{
	...
	PacketCaptureClient_enableRemoteCapture_run(engine, &action);
}
...
static int local_read_loop( max_engine_t* engine, FILE* file )
{
	...
	while( !g_shutdown )
	{
		...
		PacketCaptureClient_readCaptureData_actions_t action = {
			.param_len = frames_len,
			.outstream_toCpu = data,
		};
		PacketCaptureClient_readCaptureData_run(engine, &action);
		...
		for( int i=0; i<frames_len; i++ )
		{
			...
			if( sof )
			{
				...
				{
					logf_info("WARNING: Using system time for this packet due to invalid DFE time stamp (doubt=%d, valid=%d, value=%"PRIu64")\n",
							  timestamp_get_doubt(timestamp),
							  timestamp_is_valid(timestamp),
							  timestamp_get_value(timestamp));
